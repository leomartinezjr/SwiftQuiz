...
class QuizManager {
    ...
    func refreshQuiz(){ // pega um a quia aleatorio dos quizeS
        let randomIndex = Int (arc4random_uniform(UInt32(quizes.count)))
        let quizData = quizes[randomIndex]
        
        
        quiz = Quiz(question: quizData.question, options: quizData.options, correctedAnswer: quizData.correctAnswer) // aqui que faz a juncao da lista e cconstruir em uma
        }
    
    
    func validateAnswer(index: Int)  { // aumenta um
        _totalAnswers += 1
        if quiz.validateOption(index){ // mostra se a a respata esta correta valida index com o index do quia
